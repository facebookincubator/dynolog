...
static void update_task_count_output(struct bpf_perf_event_value* diff_val, struct bperf_thread_data *data, __u64 now) {
  int i;
  data->runtime_until_schedin += now - data->schedule_time;
  data->schedule_time = now;
  for (i = 0; i < BPERF_MAX_GROUP_SIZE && i < event_cnt; i++) {
    data->events[i].offset += diff_val[i].counter;
...
static void update_task_event_output(struct bperf_thread_data *data, __u64 now, __u32 tid) {
...
static void update_task_output(struct bpf_perf_event_value* diff_val, struct task_struct *prev, struct task_struct *next) {
  ...
  if (prev_data) {
    update_task_count_output(diff_val, prev_data, now);
  }
  if (!next)
    return;
  ...
  if (!next_data)
    return;
  next_data->schedule_time = now;
  update_task_event_output(next_data, now, next->pid);
}
...
int BPF_PROG(bperf_read_trigger) {
  ...
  return bperf_leader_prog(bpf_get_current_task_btf(), NULL);
}
...
int BPF_PROG(bperf_register_thread, struct bpf_map *map) {
  ...
  now = bpf_ktime_get_ns();
  data->schedule_time = now;
  update_task_event_output(data, now, tid);
  return 0;
}
...
static int __always_inline _pmu_enable_exit(struct pmu *pmu)
{
  ...
  now = bpf_ktime_get_ns();
  bperf_leader_prog(task, NULL);
  update_task_event_output(data, now, pid);
  return 0;
}
