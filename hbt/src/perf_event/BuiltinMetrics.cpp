      ...
      for (const auto& cache_result : kHWCacheResultTypes) {
        pmu_manager->addEvent(
            std::make_shared<EventDef>(
                ...
                EventDef::Encoding{
                    .code =
                        constructEncoding(cache_type, cache_op, cache_result)},
...
std::shared_ptr<Metrics> makeAvailableMetricsForCpu(const CpuInfo& cpu_info) {
  ...
  metrics->add(
      std::make_shared<MetricDesc>(
          ...
          50 *
              (2.7 *
               1'000'000), // About 1000 / 50 = 20 per sec in a 2.7 Ghz CPU.
          System::Permissions{},
          std::vector<std::string>{}));
              ...
              {CpuArch::BDX,
               EventRefs{
                   EventRef{
                       ...
                       EventExtraAttr::makeCountAll(),
                       {}},
                   // BW requires require special countermasks which is
          ...
          std::map<TOptCpuArch, EventRefs>{
              {CpuArch::SKX,
               EventRefs{
                   ...
                   // CPU_CLK_UNHALTED.THREAD_ANY and
                   // CPU_CLK_UNHALTED.THREAD_P_ANY
...
void addArmCoreMetrics(std::shared_ptr<Metrics>& metrics) {
  ...
  metrics->add(
      std::make_shared<MetricDesc>(
          ...
          // Not all ARM platforms expose L3 cache events via armv8_pmuv3.
          // Hypervisor PMU event filtering (e.g. on AWS Graviton 5) may
          // hide these even when the hardware supports them. The caller
          // must handle perf_event_open failure gracefully.
          std::map<TOptCpuArch, EventRefs>{
              {CpuArch::NEOVERSE_V2,
               EventRefs{EventRef{
                   "l3d_cache_refill",
  ...
  metrics->add(
      std::make_shared<MetricDesc>(
          ...
          // FP_*_SPEC are implementation-defined events that may be
          // filtered by hypervisors. The caller must handle
          // perf_event_open failure gracefully.
          std::map<TOptCpuArch, EventRefs>{
              {CpuArch::NEOVERSE_V2,
               EventRefs{EventRef{
                   "FP_HP_SPEC",
  ...
  metrics->add(
      std::make_shared<MetricDesc>(
          ...
          // Not all ARM platforms expose last-level cache miss events via
          // armv8_pmuv3. Hypervisor PMU event filtering (e.g. on AWS
          // Graviton 5) may hide these even when the hardware supports
          // them. The caller must handle perf_event_open failure gracefully.
          std::map<TOptCpuArch, EventRefs>{
              {CpuArch::NEOVERSE_V2,
               EventRefs{EventRef{
                   "ll_cache_miss_rd",
