...
class Monitor {
 ...
 private:
  ...
  bool readUncoreCountsWithCpuAffinity_(
      const std::set<CpuId>& cpus,
      std::map<
          CpuId,
          std::map<ElemId, std::vector<TUncoreCountReader::ReadValues>>>&
          perCpuResults) const {
 ...
 public:
  ...
  readAllUncoreCountsPerPerfEventOnLocalCpu() const {
    ...
    if (!readUncoreCountsWithCpuAffinity_(cpus, perCpuResults)) {
      return {};
    }
  ...
  std::map<
      ...
      std::map<ElemId, std::vector<TUncoreCountReader::ReadValues>>>
  readAllUncoreCountsPerPerfEventByScope() const {
    std::lock_guard<std::mutex> lock{mutex_};

    // First, collect all CPUs that need to be read and
    // build a mapping from CPU to all scopes it belongs to
    ...
    if (!readUncoreCountsWithCpuAffinity_(allCpus, perCpuResults)) {
      return {};
    }

    // Now group the results by scope
    std::map<
        perf_event::uncore_scope::Scope,
        std::map<ElemId, std::vector<TUncoreCountReader::ReadValues>>>
        groupedResults;
  ...
  std::shared_ptr<TUncoreCountReader> emplaceUncoreCountReader(
      ...
      std::vector<perf_event::uncore_scope::Scope> scopes) {
    try {
      std::lock_guard<std::mutex> lock{mutex_};
