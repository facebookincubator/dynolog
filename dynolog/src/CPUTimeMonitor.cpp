...
void CPUTimeMonitor::processProcUsage(
  ...
  for (const auto& [targetId, cpuSet] : targetToCpuSet_) {
    ...
    double idleCores =
        static_cast<double>(newCt.i - lastCt.i) * msPerJiffy / wallDelta;
    double softirqCores =
        static_cast<double>(newCt.y - lastCt.y) * msPerJiffy / wallDelta;
    double iowaitCores =
        static_cast<double>(newCt.w - lastCt.w) * msPerJiffy / wallDelta;
    double hardirqCores =
        static_cast<double>(newCt.x - lastCt.x) * msPerJiffy / wallDelta;
