...
InterruptStats InterruptStatsMonitor::getInterruptStatsPerMinute() {
  return interruptStats_[static_cast<int>(Granularity::MINUTE)];
}

InterruptStats InterruptStatsMonitor::getInterruptStatsPerSecond() {
  return interruptStats_[static_cast<int>(Granularity::SECOND)];
}
...
void InterruptStatsMonitor::tick(TMask mask) {
  ...
  if (TTicker::is_major_tick(mask)) {
    ...
    interruptStats_.insert_or_assign(
        static_cast<int>(Granularity::MINUTE), freshStats);
  } else if (TTicker::is_minor_tick(mask)) {
    InterruptStats freshStats = interruptsRefresh();
    interruptStats_.insert_or_assign(
        static_cast<int>(Granularity::SECOND), freshStats);
  }
}

InterruptStats InterruptStatsMonitor::interruptsRefresh() {
  std::string fullPath = rootDir_ + "/proc/interrupts";
  folly::small_vector<folly::StringPiece> v;
  ...
  try {
    if (!std::filesystem::exists(fullPath)) {
      XLOG(ERR) << "Path " << fullPath << " does not exist";
      return stats;
    }
    std::ifstream file(fullPath);
    if (!file.is_open()) {
      XLOG(ERR) << "Failed to open the file  " << fullPath;
      return stats;
    }
    ...
    while (file.getline(buf, bufferSize)) {
      ...
      if (strstr(buf, "TLB shootdowns")) {
        folly::split(' ', buf, v, true);
        int64_t tlbshootdowns = 0;
        size_t valueCount = 0;
        for (const auto& tlbshootdownStr : v) {
          folly::tryTo<int64_t>(tlbshootdownStr)
              .then([&tlbshootdowns, &valueCount](int64_t tlbshootdown) {
        ...
        if (valueCount == cpuCount_) {
          ...
          XLOG(WARN) << "CPU count from procfs interrupts: " << valueCount
                     << " Expected: " << cpuCount_;
        }
        v.clear();
      }
      ...
      if (strstr(buf, "eth0-") || strstr(buf, "mlx5_comp")) {
        ...
        size_t valueCount = 0;
        for (const auto& eth0IntrpStr : v) {
          folly::tryTo<int64_t>(eth0IntrpStr)
              .then([&eth0IntrpRow, &valueCount](int64_t eth0Intrp) {
    ...
    XLOG(ERR) << "Error in reading the procfs interrupts file: " << fullPath
              << " Error: " << e.what();
  }
  return stats;
}
